...
def test_def_get_companies_and_vacancies_count(db_manager: DBManager):
    db_manager.connect()
    assert db_manager.get_companies_and_vacancies_count() == ([('0.75 GROUP', 20),
...
def test_get_avg_salary(db_manager: DBManager) -> None:
    db_manager.connect()
    assert db_manager.get_avg_salary() == [(79921.875,)]
    db_manager.connection_close()
...
def test_get_vacancies_with_keyword(db_manager: DBManager):
    db_manager.connect()
